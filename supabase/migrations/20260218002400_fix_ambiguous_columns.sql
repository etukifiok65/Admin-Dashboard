-- =====================================================
-- Fix Ambiguous Column References in RPC Function
-- Migration: 20260218002400_fix_ambiguous_columns.sql
-- Purpose: Fully qualify all column references in create_broadcast_notification
-- =====================================================
...
BEGIN
  ...
  RETURNING public.broadcast_notifications.id INTO v_notification_id;
  ...
  IF p_recipient_type = 'patients' OR p_recipient_type = 'both' THEN
    ...
    SELECT v_notification_id, public.patients.id FROM public.patients WHERE public.patients.is_active = TRUE;
  ...
  IF p_recipient_type = 'providers' OR p_recipient_type = 'both' THEN
    ...
    SELECT v_notification_id, public.providers.id FROM public.providers WHERE public.providers.is_active = TRUE;
  ...
  WHERE public.broadcast_notification_recipients.broadcast_notification_id = v_notification_id;

  -- Update notification with recipient count
  UPDATE public.broadcast_notifications
  SET public.broadcast_notifications.total_recipients = v_total_recipients
  WHERE public.broadcast_notifications.id = v_notification_id;
